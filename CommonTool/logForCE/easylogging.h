...
namespace easyloggingpp {
	...
	struct Level : private StaticClass {
	public:
		enum {
	...
	struct ConfigurationType : private StaticClass {
	public:
		enum {
	...
	struct Aspect : private StaticClass {
	public:
		enum {
	...
	class Constants : private NoCopy {
	public:
...
#   endif // _ELPP_OS_UNIX
	...
	namespace utilities {
		class StringUtilities : private StaticClass {
		public:
...
#define ELPP_StringUtils utilities::StringUtilities
		class OSUtilities : private StaticClass {
		public:
			...
			static int mkDir(const char *__CLR_OR_THIS_CALL path)
			{
				...
				while(*tmp)
				{
					...
					{
						*tmp = '\0';
						DWORD fileAttr = GetFileAttributesA((LPCSTR) path);
		...
		//
		class LogManipulator : private StaticClass {
		public:
		...
		//
		class DateUtilities : private StaticClass {
		public:
#if _ELPP_OS_WINDOWS
...
#   endif // defined(_MSC_EXTENSIONS)
			...
			static std::string getDateTime(const std::string& bufferFormat_, unsigned int type_, Constants* constants_, unsigned int milliSecondOffset_ = 1000) {
				long milliSeconds = 0;
				...
				if ((type_ & constants_->kDateTime) || (type_ & constants_->kDateOnly)) {
					sprintf(dateBufferOut_,"%d-%d-%d",sysTime.wYear,sysTime.wMonth,sysTime.wDay);//"2010-09-21"
				}
				if ((type_ & constants_->kDateTime) || (type_ & constants_->kTimeOnly)) {
					sprintf(dateBufferOut_,"%d:%d:%d",sysTime.wHour,sysTime.wMinute,sysTime.wSecond);//"2010-09-21"
				}
#endif // _ELPP_OS_UNIX
	...
	class Configurations : public Registry<Configuration, Configuration::Predicate> {
	public:
		...
		void set(unsigned int level_, unsigned int configurationType_, const std::string& value_) {
			...
			Configuration* conf_ = get(level_, configurationType_);
			if (conf_ == NULL) {
				registerNew(new Configuration(level_, configurationType_, value_));
			} else {
...
#elif _ELPP_OS_WINDOWS
			setAll(ConfigurationType::ELPP_Filename, "Nand/myeasylog.log");
#endif // _ELPP_OS_UNIX
		...
		class Parser : private StaticClass {
		public:
	...
	private:
		...
		inline void set(Configuration* conf_) {
	...
	public:
		TypedConfigurations(const Configurations& configurations, Constants* constants_) :
	...
	class Logger {
	public:
		Logger(const std::string& uniqueIdentifier_, Constants* constants_) :
		  ...
		  Logger(const std::string& uniqueIdentifier_, Constants* constants_, const Configurations& configurations) :
		  id_(uniqueIdentifier_),
			  constants_(constants_),
			  typedConfigurations_(NULL),
			  stream_(new std::stringstream()){
		  ...
		  void configure(const Configurations& configurations_) {
			  ...
			  typedConfigurations_ = new TypedConfigurations(base_, constants_);
			  configured_ = true;
		  }
	...
	public:
		TypedConfigurations* typedConfigurations_;
	private:
		std::stringstream* stream_;
		...
		class LogCounter : private NoCopy {
		public:
		...
		class RegisteredLoggers : public Registry<Logger, Logger::Predicate> {
		public:
			RegisteredLoggers(void) :
			  constants_(new Constants()),
				  ...
				  counters_(new RegisteredCounters()) {
					  Configurations conf;
					  conf.setToDefault();
		...
		private:
			...
			Logger* get(const std::string& id_) {
				Logger* logger_ = Registry<Logger, Logger::Predicate>::get(id_);
				if (logger_ == NULL) {
			...
			inline void unregister(Logger*& logger_) {
				Registry<Logger, Logger::Predicate>::unregister(logger_);
			}
		...
		extern SmartPointer<RegisteredLoggers> registeredLoggers;
#if defined(_ELPP_STL_LOGGING)
...
#   endif // _ENABLE_EASYLOGGING
		...
		private:
			...
			bool proceed_;
			easyloggingpp::Constants* constants_;
			friend class Logger;
			...
			void buildAndWriteLine(void) {
				...
				if (f_ & constants_->kAppName) {
					...
					easyloggingpp::utilities::LogManipulator::updateFormatValue(fs_, v_, currLine_, constants_);
				}
				...
				if ((f_ & constants_->kDateOnly) || (f_ & constants_->kTimeOnly) || (f_ & constants_->kDateTime)) {
					v_ = easyloggingpp::utilities::DateUtilities::getDateTime(dateFormat,
				...
				if (severity_ == Level::ELPP_VERBOSE && f_ & constants_->kVerboseLevel) {
					...
					easyloggingpp::utilities::LogManipulator::updateFormatValue(fs_, tempss_.str(), currLine_, constants_);
				}
				// Log message
				if (f_ & constants_->kLogMessage) {
					fs_ = constants_->LOG_MESSAGE_FORMAT_SPECIFIER;
					easyloggingpp::utilities::LogManipulator::updateFormatValue(fs_, logger_->stream()->str(), currLine_, constants_);
				}
				log();
			}
			void log(void) {
	...
	class VersionInfo : private StaticClass {
	public:
	...
	//
	class Loggers : private StaticClass {
	public:
		static inline Logger* getLogger(const std::string& identifier_) {
			return registeredLoggers->get(identifier_);
		}

		static inline Logger* reconfigureLogger(Logger* logger_, Configurations& configurations_) {
			if (!logger_) return NULL;
			if (!registeredLoggers->constants_->CLEAN_FILES_ON_NEXT_PARSE) {
				registeredLoggers->constants_->CLEAN_FILES_ON_NEXT_PARSE = configurations_.contains(ConfigurationType::ELPP_Filename);
			}
		...
		static inline void setApplicationArguments(int argc, char** argv) {
			registeredLoggers->setApplicationArguments(argc, argv);
		}
		...
		static inline void reconfigureAllLoggers(Configurations& configurations_) {
			...
			for (unsigned int i = 0; i < registeredLoggers->count(); ++i) {
				Logger* l = registeredLoggers->at(i);
				Loggers::reconfigureLogger(l, configurations_);
			}
			registeredLoggers->constants_->CLEAN_FILES_ON_NEXT_PARSE = false;
		}
		...
		static inline void reconfigureAllLoggers(unsigned int configurationType_, const std::string& value_) {
			if (!registeredLoggers->constants_->CLEAN_FILES_ON_NEXT_PARSE &&
				configurationType_ == ConfigurationType::ELPP_Filename) {
					registeredLoggers->constants_->CLEAN_FILES_ON_NEXT_PARSE = true;
			}
			for (unsigned int i = 0; i < registeredLoggers->count(); ++i) {
				Logger* l = registeredLoggers->at(i);
		...
		static inline void getAllLogIdentifiers(std::vector<std::string>& listOfIds) {
			listOfIds.clear();
			for (unsigned int i = 0; i < registeredLoggers->count(); ++i) {
				listOfIds.push_back(registeredLoggers->at(i)->id());
			}
		}
		...
		class ConfigurationsReader : private StaticClass {
		public:
		...
		private:
			static inline TypedConfigurations* constConf(Logger* logger_) {
...
#   elif _ELPP_OS_WINDOWS
#      define _ELPP_GET_CURR_TIME(tm) easyloggingpp::utilities::DateUtilities::gettimeofday(tm);
#   endif
#   define START_FUNCTION_LOG "Executing [" << __func__ << "]"
#   define TIME_OUTPUT "Executed [" << __func__ << "] in [" <<                    \
	easyloggingpp::utilities::DateUtilities::formatMilliSeconds(        \
	easyloggingpp::utilities::DateUtilities::getTimeDifference(functionEndTime, functionStartTime)) << "]"
...
#define _ELPP_LOG_WRITER(_logger, _level) easyloggingpp::Writer(\
	_logger, easyloggingpp::Aspect::Normal, _level, __func__, __FILE__, __LINE__)
#define _ELPP_LOG_WRITER_COND(_c, _logger, _level) if (_c) easyloggingpp::Writer(\
	_logger, easyloggingpp::Aspect::Conditional, _level, __func__, __FILE__, __LINE__, _c)
#define _ELPP_LOG_WRITER_N(_n, _logger, _level) if (easyloggingpp::registeredLoggers->validateCounter(\
	__FILE__, __LINE__, _n)) easyloggingpp::Writer(_logger, easyloggingpp::Aspect::Interval,\
...
#define CVERBOSE(vlevel_, loggerId) easyloggingpp::Writer(loggerId, easyloggingpp::Aspect::Normal,       \
...
#define CVERBOSE_IF(condition_, vlevel_, loggerId) if (condition_) easyloggingpp::Writer(loggerId, easyloggingpp::Aspect::Conditional,     \
...
#define CVERBOSE_EVERY_N(interval_, vlevel_, loggerId) if (easyloggingpp::registeredLoggers->validateCounter(__FILE__, __LINE__, interval_)) \
	easyloggingpp::Writer(loggerId, easyloggingpp::Aspect::Interval,   \
...
#define _ELPP_COUNTER easyloggingpp::registeredLoggers->counters()->get(__FILE__, __LINE__)
