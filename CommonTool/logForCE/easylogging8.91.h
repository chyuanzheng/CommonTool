...
public:
	...
	// Gets current GetWorkPath.
	static  bool getWorkPath( std::string &path)
	{

		wchar_t tWorkPath[MAX_PATH];
		if(::GetModuleFileName(   NULL,   tWorkPath,   MAX_PATH   ) == 0)
		{
			return false;
		}
		wchar_t   *p   =   wcsrchr(   tWorkPath,   '\\'   );     
		*(p+1)   =   0; 

		char workPath[MAX_PATH];
		wcstombs(workPath,tWorkPath,MAX_PATH);
		path = workPath;
		return true;
	}
...
//         }
        ...
        while (currPath_ != NULL) {
			...
			mbstowcs(dest,buildingPath_.c_str(),MAX_PATH);
...
#   endif // defined(_MSC_EXTENSIONS)
    ...
    static std::string getDateTime(const std::string& bufferFormat_, unsigned int type_, internal::Constants* constants_, std::size_t milliSecondOffset_ = 1000) {
        ...
        if ((type_ & constants_->kDateTime) || (type_ & constants_->kTimeOnly)) {
			sprintf(dateBufferOut_,"%d:%d:%d:%d",sysTime.wHour,sysTime.wMinute,sysTime.wSecond,sysTime.wMilliseconds);//"2010-09-21"
        }
#endif // _ELPP_OS_UNIX
...
#   endif // _ELPP_NDK
        ...
        setAll(ConfigurationType::ToStandardOutput, "false");
...
public:
    ...
    void insertFilename(unsigned int level_, const std::string& fname_, bool forceNew = false) {
		...
		internal::utilities::OSUtils::getWorkPath(fnameFull);
		fnameFull += fname_;
