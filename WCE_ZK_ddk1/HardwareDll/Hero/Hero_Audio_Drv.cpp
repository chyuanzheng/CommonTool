...
S16 Geq_UserModeTable[GEQ_MAXBAND][7];
/*-----------------Geq config end----------------------*/
...
------------------------------------------------------------------------*/
void Hero_EasyProgrammWrite(U32 Data)
{
	U8 Buffer[6];
	...
	int i=0;
	while((Buffer[0]!=0)&&(Buffer[1]!=0)&&(Buffer[2]!=0)&&i++<1000)   //check if the easyprogramm routing is able to write 
	{
		Buffer[0]=0xF2;
		Buffer[1]=(U8)(ADSP_X_EasyP_Index>>8);	  //addr is 2 bytes
		Buffer[2]=(U8)ADSP_X_EasyP_Index;
		...
		if(!I2C1_ReadData(TEF663x_SlaveAddr,Buffer,3))	 //start to read to buffer
		{
			break;
		}
	}
...
static void Audio_DrvGetAddress(eAudio_Feature_t From, S32 *AddressPointer)
{
	static const U16 OutputAddress[] =
	{
		...
		ADSP_X_AUB2_out1_REL,                                /*ultrabass        =52*/
	};
...
void Audio_DrvSetStreamInput(Audio_Stream_t stream,eAudio_InputSource Input/*,U8 mode*/)
{
	...
	switch(stream)
	{
		case PrimaryStr:
			...
			if (Input == INPUT_SOURCE_RADIO)
			{
				Audio_DrvSetSourceScaling(stream, -2);
			}
			else if(Input == INPUT_SOURCE_AIN0)
			{
				Audio_DrvSetSourceScaling(stream, 0);
			}
			else if (Input == INPUT_SOURCE_IIS0)
			{
				Audio_DrvSetSourceScaling(stream, -8);
			}
			else if (Input == INPUT_SOURCE_IIS1)
			{
				Audio_DrvSetSourceScaling(stream, -8);
			}
			else if (Input == INPUT_SOURCE_AIN1)
			{
				Audio_DrvSetSourceScaling(stream, 0);
			}
			else if (Input == INPUT_SOURCE_AIN2)
			{
				Audio_DrvSetSourceScaling(stream, 0);
			}
			else
			{
				Audio_DrvSetSourceScaling(stream, -16);
			}
			break;
		case SecondaryStr:
			if(Input!=INPUT_SOURCE_AIN3) //Ain3 not avilable for primary and sencond
...
{
	Hero_EasyProgrammWrite(ADSP_EASYP_PCHIME_IMGpreset);
}
...
void Audio_DrvFaderSet(eAudioChannel Channel, U32 Level)
{
    ...
    default:
     ...
     WriteDataToHero(I2CDRV_Y_MEM,ADSP_Y_Vol_FadF_REL,2,Att);          

    /* Sw and Center = Att[0] (= same value as front) update */
    Temp = (Temp >> 1) + 0x400;/* Sw update */
    Att[1] = Temp;
    WriteDataToHero(I2CDRV_Y_MEM,ADSP_Y_Vol_FadSwL_REL,2,Att);
}
...
void Audio_DrvLimiterConfigurationSel(U8 config)
{
  if(1==config) {/*  configuration 1 */ 	
        Hero_EasyProgrammWrite(ADSP_EASYP_Limit_LimitRSOnSec);
  }
  else if(2==config){  /*configuration 2 */
        Hero_EasyProgrammWrite(ADSP_EASYP_Limit_LimitRSOnRear);
  }
}
/*-----------------------------------------------------------------------------
...
{
	Hero_EasyProgrammWrite(ADSP_EASYP_Chime_Enable); //see the para in UM
}
/*-----------------------------------------------------------------------
...
void Audio_DrvDisableChime(void)
{
	...
	Hero_EasyProgrammWrite(ADSP_EASYP_ChimePDC_Disable); //see the para in UM,and at the same time the PDC 1 will be also disable
}
/*-----------------------------------------------------------------------
Input:	NULL		
Output:	NULL		
...
void Audio_DrvSinGenSwitch(U8 OnOff)
{
	...
	{
		Hero_EasyProgrammWrite(ADSP_EASYP_SinGen_Enable);
	}
	else//disable the sin generator, also with the noise generator and checksum function
	{
		Hero_EasyProgrammWrite(ADSP_EASYP_NoiseGen_SinGen_CheckSum_Disable);
	}
}
/*-----------------------------------------------------------------------
Input:	OnOff: 0=off, others=on	
...
void Audio_DrvNoiseGenSwitch(U8 OnOff)
{
    ...
    {
		Hero_EasyProgrammWrite(ADSP_EASYP_NoiseGen_Enable);
    }
...
S8 Audio_DrvSoftMuteSwitch(U16 Channel,U8 OnOff)
{
	...
	MuteData = OnOff? 0x0000/*mute on*/ : 0x7FF/*mute off*/;

	if(Channel&Audio_PCHANNEL)		//overall primary channel
	{
		WriteHero_MEM_1Word(I2CDRV_Y_MEM,ADSP_Y_Mute_P,MuteData);
	}
	if(Channel&Audio_PFLCHANNEL)		//front left channel
	{
		WriteHero_MEM_1Word(I2CDRV_Y_MEM,ADSP_Y_MuteSix_FL,MuteData);
	}
	if(Channel&Audio_PFRCHANNEL)		//front right channel
	{
		WriteHero_MEM_1Word(I2CDRV_Y_MEM,ADSP_Y_MuteSix_FR,MuteData);
	}
	if(Channel&Audio_PRLCHANNEL)		//rear left channel
	{
		WriteHero_MEM_1Word(I2CDRV_Y_MEM,ADSP_Y_MuteSix_RL,MuteData);
	}
	if(Channel&Audio_PRRCHANNEL)		//rear right channel
	{
		WriteHero_MEM_1Word(I2CDRV_Y_MEM,ADSP_Y_MuteSix_RR,MuteData);
	}
	if(Channel&Audio_SWLCHANNEL)		//subwoofer left channel
	{
		WriteHero_MEM_1Word(I2CDRV_Y_MEM,ADSP_Y_MuteSix_SwL,MuteData);
	}
	if(Channel&Audio_SWRCHANNEL)		//subwoofer right channel
	{
		WriteHero_MEM_1Word(I2CDRV_Y_MEM,ADSP_Y_MuteSix_SwR,MuteData);
	}
	if(Channel&Audio_SCHANNEL)		//overall secondary channel
	{
		WriteHero_MEM_1Word(I2CDRV_Y_MEM,ADSP_Y_Mute_S,MuteData);
	}
	if(Channel&Audio_NCHANNEL)		//overall navigation channel
	{
		WriteHero_MEM_1Word(I2CDRV_Y_MEM,ADSP_Y_Mute_N,MuteData);
	}
	if(Channel&Audio_TCHANNEL)		//overall phone channel
	{
...
void Audio_DrvSoftMuteConfig(U16 Channel,U16 Ta,U16 Tr)
{
	...
	case Audio_PCHANNEL:
		...
		break;
	case Audio_PFLCHANNEL:
	case Audio_PFRCHANNEL:
	case Audio_PRRCHANNEL:	
	case Audio_PRLCHANNEL:	
	case Audio_SWLCHANNEL:
	case Audio_SWRCHANNEL:
		Data_A=Data_A<<1;
		Data_R=Data_R<<1;
...
static const sMemAddr_Data LoudnessSwitchTab[] = {
	//{I2CDRV_Y_MEM,	ADSP_Y_Loudf_MaxBstB,	0x0096},
	{I2CDRV_Y_MEM,	ADSP_Y_Loudf_MaxBstB,	0x012B},
	{I2CDRV_Y_MEM,	ADSP_Y_Loudf_MaxBstT,	0x012B},
...
S8 Audio_DrvGSASwitch(U8 OnOff)
{
	if(OnOff)		 //enable
	{
		Hero_EasyProgrammWrite(ADSP_EASYP_GSA_Enable);
	}
	else			 //disable
	{
		Hero_EasyProgrammWrite(ADSP_EASYP_GSA_Disable);
	}
	return SUCCESS;
}
/*-----------------------------------------------------------------------
...
{
	Hero_EasyProgrammWrite(ADSP_EASYP_ClickClack_Disable); //see the para in UM
}
/*-----------------------------------------------------------------------
...
U8 Audio_DrvSetI2SOutMode(U8 I2SNum, U8 Mode)
{
	U8 Buffer[3];    
...
/*+12db ~ -36db*/
const S32 tabHero_PhonNavVolumeValue[]=
{
/*12 ~3db*/
...
/*-28 ~-36db*/
0x0FD7, 0x0FDC, 0x0FE0, 0x0FE3, 0x0FE6, 0x0FE9, 0x0FEC, 0x0FEE, 0x0FF0, 0x0FF1, 
0x0FF2, 0x0FF3, 0x0FF4, 0x0FF5, 0x0FF6, 0x0FF7, 0x0FF8, 0x0FF9, 0x0FFA, 0x0FFB,
0x0FFC, 0x0FFD, 0x0FFE, 0x0FFF, 0x0FFF, 0x0FFF, 0x0FFF, 0x0FFF, 0x0FFF, 0x0FFF,
};
/*-----------------------------------------------------------------------
...
{
	S32 Volume_Data01,Volume_Data02;
	//double ExpData;
	U8 Step;
     U32 addr;
	switch(Channel)
	...
	if((Channel==PrimaryStr)||(Channel==SecondaryStr))		 //set primary channel volume 
	{
		...
		else//see volume calcute  
		{
			...
			//Volume_Data01=table_dB2Lin[Step];
			//Volume_Data01*=256;
			Volume_Data01 = 2048*256*pow(10, (Volume-12.04)/20);
	...
	else if((Channel==NavigationStr)||(Channel==PhoneStr))
	{
		...
		*/
		...
		//Volume_Data01 = 2048/4*pow(10, (Volume)/20);
...
S8 Audio_DrvConfigGeqBand(U8 Band,S32 Geq_DescaleData)
{
...
#if GEQ_9BAND_ENABLE
	if((Band<GEQ_BAND4)||(Band==GEQ_BAND6)||(Band==GEQ_BAND7)) //double presicion
	{
		for(i=0;i<6;i++)
		{
...
#else
	if((Band<GEQ_BAND4)) //double presicion
	{
		for(i=0;i<6;i++)
		{
			Buffer[i]=Geq_UserModeTable[Band][i+1];//get the band data Y memory format
	...
	switch(Band)
	{
		...
		case GEQ_BAND6:
		{
			WriteScratchDataToHero(I2CDRV_Y_MEM,ADSP_Y_EQ4_b10L_REL, ADSP_EASYP_UpdateCoeff_Coeff7, Buffer);
			Data=Geq_DescaleData; //write the descale data
			WriteHero_MEM_1Word(I2CDRV_Y_MEM,ADSP_Y_Vol_DesScalGEq,Data); 				
			break;
		}
		case GEQ_BAND7:
		{
			WriteScratchDataToHero(I2CDRV_Y_MEM,ADSP_Y_EQ4_b20L_REL, ADSP_EASYP_UpdateCoeff_Coeff7, Buffer);
			Data=Geq_DescaleData; //write the descale data
			WriteHero_MEM_1Word(I2CDRV_Y_MEM,ADSP_Y_Vol_DesScalGEq,Data);  				
			break;
		}		
		case GEQ_BAND8:
		{
			WriteScratchDataToHero(I2CDRV_Y_MEM,ADSP_Y_EQ4_b30_REL, ADSP_EASYP_UpdateCoeff_Coeff4, Buffer);
			Data=Geq_DescaleData; //write the descale data
			WriteHero_MEM_1Word(I2CDRV_Y_MEM,ADSP_Y_Vol_DesScalGEq,Data);  				
			break;
		}		
		case GEQ_BAND9:
		{
			WriteScratchDataToHero(I2CDRV_Y_MEM,ADSP_Y_EQ4_b40_REL, ADSP_EASYP_UpdateCoeff_Coeff4, Buffer);
			Data=Geq_DescaleData; //write the descale data
			WriteHero_MEM_1Word(I2CDRV_Y_MEM,ADSP_Y_Vol_DesScalGEq,Data); 				
			break;
		}	
...
#if GEQ_9BAND_ENABLE
	if((Geq_Band==GEQ_BAND1)||(Geq_Band==GEQ_BAND2)||(Geq_Band==GEQ_BAND3)||(Geq_Band==GEQ_BAND6)||(Geq_Band==GEQ_BAND7))//double precision
#else
	if((Geq_Band==GEQ_BAND1)||(Geq_Band==GEQ_BAND2)||(Geq_Band==GEQ_BAND3))//double precision
#endif
	{
...
#if GEQ_9BAND_ENABLE
	else if((Geq_Band==GEQ_BAND4)||(Geq_Band==GEQ_BAND5)||(Geq_Band==GEQ_BAND8)||(Geq_Band==GEQ_BAND9))//single precision
#else
	else if((Geq_Band==GEQ_BAND4)||(Geq_Band==GEQ_BAND5))//single precision
#endif
	{
...
S8 Audio_CalcuteGeq(sGeq_ParaDef *pGeqPara[])
{
	U8 band;
	sGeq_ParaDef Geq_Para[GEQ_MAXBAND];
	for(band=GEQ_BAND1;band<GEQ_MAXBAND;band++)
	{
		Audio_DrvCalcuteGeq(band,&Geq_Para[band]);//thrans the default data to hero data format
		for (int i=0;i<7;i++)
		{
			RETAILMSG(1, (TEXT("0x%03x ", Geq_UserModeTable[band][i])));
		}
		RETAILMSG(1, (TEXT("\r\n")));
	}
	return 1;
...
/*-----------------------------------------------------------------------
Function name:		Audio_DrvGEQ9Band_Enable	  
Input:	
         	
Output:			
Description: 	 enable 9ban GEQ
------------------------------------------------------------------------*/
void Audio_DrvGEQ9Band_Enable(void)
{
  Hero_EasyProgrammWrite(ADSP_EASYP_GraphEQ_9Band_Enable);

}

/*-----------------------------------------------------------------------
Function name:		Audio_DrvGEQ9Band_Disable	  
Input:	
         	
Output:			
Description: 	 Disable 9ban GEQ
------------------------------------------------------------------------*/
void Audio_DrvGEQ9Band_Disable(void)
{
  Hero_EasyProgrammWrite(ADSP_EASYP_GraphEQ_9Band_Disable);
}
...
Input:	Channel: can set several channel once but with the same delay time
			Audio_PFLCHANNEL 0x0001   //front left channel     
			Audio_PFRCHANNEL 0x0002	//front right channel
			Audio_PRLCHANNEL 0x0004	//rear left channel
			Audio_PRRCHANNEL 0x0008	//rear right channel
			Audio_SWLCHANNEL 0x0010	//subwoofer left channel
			Audio_SWRCHANNEL 0x0020	//subwoofer right channel
			Audio_SLCHANNEL 0x0040	//secondary left channel
			Audio_SRCHANNEL 0x0080	//secondary right channel
...
{	
	...
	if(Channel&Audio_PFLCHANNEL)
	{
		Data1-=Hero_DelayTime.PFLChannel;
		Data1+=Time;
	}
	if(Channel&Audio_PFRCHANNEL)
	{
		Data1-=Hero_DelayTime.PFRChannel;
		Data1+=Time;	    
	}
	if(Channel&Audio_PRLCHANNEL)
	{
		Data1-=Hero_DelayTime.PRLChannel;
		Data1+=Time;	    
	}
	if(Channel&Audio_PRRCHANNEL)
	{
		Data1-=Hero_DelayTime.PRRChannel;
		Data1+=Time;	    
	}
	if(Channel&Audio_SWLCHANNEL)
	{
		Data1-=Hero_DelayTime.SWLChannel;
		Data1+=Time;	    
	}
	if(Channel&Audio_SWRCHANNEL)
	{
...
Output:	PARA_ERROR=the input para is wrong
		SUCCESS=set ok 
Description: disable ultra bass
------------------------------------------------------------------------*/
S8 Audio_DrvUltraBassDisable(void)
{
	/*disable UltraBass*/
	Hero_EasyProgrammWrite(ADSP_EASYP_AUB2_Disable);
	/*UltraBass switch off*/
	WriteHero_MEM_1Word(I2CDRV_X_MEM, ADSP_X_AUB2_Mode, 0x00);
	/*connet input pointer of PEQ and Subwoofer to Loudnes output*/
	Audio_DrvPathConnect(Loudness,PEQ);
	return SUCCESS;
}
/*-----------------------------------------------------------------------
...
Output:	PARA_ERROR=the input para is wrong
		SUCCESS=set ok 
Description: set ultrabass target level
------------------------------------------------------------------------*/
void Audio_DrvUltraBassSetTargetLevel(S8 lev)
{
   S32 data=table_dB2Lin[3-lev]*4096;
   WriteHero_MEM_1Word(I2CDRV_X_MEM, ADSP_X_AUB2_TargetLevel, data);
}
/*-----------------------------------------------------------------------
Input:	lev[0,24]
Output:	PARA_ERROR=the input para is wrong
		SUCCESS=set ok 
Description: set ultrabass enhance gain
------------------------------------------------------------------------*/
void Audio_DrvUltraBassSetEnhanceGain(S8 gain)
{
   S32 data=(128*4096*2048)/table_dB2Lin[gain];
   WriteHero_MEM_1Word(I2CDRV_X_MEM, ADSP_X_AUB2_AGCMaxGain, data);
}
/*-----------------------------------------------------------------------
Input:	NULL
Output:	PARA_ERROR=the input para is wrong
		SUCCESS=set ok 
Description: Enable UltraBass	
------------------------------------------------------------------------*/
S8 Audio_DrvUltraBassEnable(void)
{
	/*enable UltraBass*/
	Hero_EasyProgrammWrite(ADSP_EASYP_AUB2_Enable);
	/*UltraBass switch on*/
	WriteHero_MEM_1Word(I2CDRV_X_MEM, ADSP_X_AUB2_Mode, 0x03);
	/*connet input pointer of PEQ and Subwoofer to ultrabass output*/
	Audio_DrvPathConnect(UltraBass,PEQ);
	return SUCCESS;
}
/*-----------------------------------------------------------------------
Input:	NULL
Output:	PARA_ERROR=the input para is wrong
		SUCCESS=set ok 
Description: Enable ALE	
------------------------------------------------------------------------*/
S8 Audio_DrvALEEnable(void)
{
	/*ALE Enable*/
	Hero_EasyProgrammWrite(ADSP_EASYP_ALE_Enable);
	/*ALE Switch on*/
	WriteHero_MEM_1Word(I2CDRV_X_MEM, ADSP_X_ALE_PAR_OnOff,0x01);
     /*conect path*/
	WriteHero_MEM_1Word(I2CDRV_X_MEM, ADSP_X_ALE_InPntr,ADSP_X_PChannelMode_OutL_REL);
	WriteHero_MEM_1Word(I2CDRV_X_MEM, ADSP_X_CompExp_InPntr,ADSP_X_ALE_OutL_REL);
	return SUCCESS;
}
/*-----------------------------------------------------------------------
Input:	NULL
Output:	PARA_ERROR=the input para is wrong
		SUCCESS=set ok 
Description: Disable ALE	
------------------------------------------------------------------------*/
S8 Audio_DrvALEDisable(void)
{ 
	/*ALE Switch on*/
	WriteHero_MEM_1Word(I2CDRV_X_MEM, ADSP_X_ALE_PAR_OnOff,0x00);
	/*ALE Enable*/
	Hero_EasyProgrammWrite(ADSP_EASYP_ALE_Disable);
	/*disconect path*/
	WriteHero_MEM_1Word(I2CDRV_X_MEM, ADSP_X_CompExp_InPntr,ADSP_X_PChannelMode_OutL_REL);
	return SUCCESS;
}
