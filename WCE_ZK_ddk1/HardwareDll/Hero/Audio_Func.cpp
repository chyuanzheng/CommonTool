...
void Audio_SetVolume(Audio_Stream_t Channel,U8 Lev)
{
	...
	if (Channel == NavigationStr)
	{
		Vol = tblNaviVolLevelTrans[Lev];
	}
...
* FUNCTION    :set bass                     
-------------------------------------------------------------------------------*/
void Audio_SetBass(Audio_Stream_t Stream,S8 *pGain)
{    
      /*check parameters Gain*/
   	if(*pGain<(0-TEF663x_TONEGAINMAX))
		*pGain = (0-TEF663x_TONEGAINMAX);
	else if(*pGain > TEF663x_TONEGAINMAX)
		*pGain =  TEF663x_TONEGAINMAX;
	/*Set Treble Fc=100Hz*/		
	//Audio_DrvBassSet(Stream, *pGain,BASS_CENTER_FRQ);
	Audio_DrvBassSetGain(Stream, *pGain);
	//update Vol_DesScalBMTP
	if(*pGain>Audio_MaxToneGain)
	{
          Audio_MaxToneGain=*pGain;
...
* FUNCTION    :set mid                    
-------------------------------------------------------------------------------*/
void Audio_SetMid(Audio_Stream_t Stream,S8 *pGain)
{      
      /*check parameters Gain*/
      if(*pGain<(0-TEF663x_TONEGAINMAX))
		*pGain = (0-TEF663x_TONEGAINMAX);
		...
		*pGain =  TEF663x_TONEGAINMAX;
	/*Set Treble Fc=500Hz*/
	Audio_DrvMidSet(Stream, *pGain,MID_CENTER_FRQ);
     //update Vol_DesScalBMTP
	if(*pGain>Audio_MaxToneGain)
	{
          Audio_MaxToneGain=*pGain;
...
-------------------------------------------------------------------------------*/
void Audio_SetTreb(Audio_Stream_t Stream,S8 *pGain)
{     
       /*check parameters Gain*/
       if(*pGain<(0-TEF663x_TONEGAINMAX))
		*pGain = (0-TEF663x_TONEGAINMAX);
	...
	Audio_DrvTrebSet(Stream, *pGain,TREBLE_CENTER_FRQ);
	//update Vol_DesScalBMTP
	if(*pGain>Audio_MaxToneGain)
	{
          Audio_MaxToneGain=*pGain;
...
void Audio_SetChimeMode(U8 Mode)
{
    switch(Mode)
	{
		...
		{
			Hero_EasyProgrammWrite(ADSP_EASYP_Chime_PSBle1); //see the para in UM
			break;
		}
		case CHIMEMODE_FASTENSEATBELT:
		{
			Hero_EasyProgrammWrite(ADSP_EASYP_Chime_PSFsb); //see the para in UM
			break;
		}
		case CHIMEMODE_KEYINIGNITION:
		{
			Hero_EasyProgrammWrite(ADSP_EASYP_Chime_PSKii); //see the para in UM
			break;
		}
		case CHIMEMODE_CRUISECONTROL:
		{
			Hero_EasyProgrammWrite(ADSP_EASYP_Chime_PSCci); //see the para in UM
			break;
		}
		case CHIMEMODE_FRONTALERT:
		{
			Hero_EasyProgrammWrite(ADSP_EASYP_Chime_PSAlt); //see the para in UM
			break;
		}
		case CHIMEMODE_USER:
		{
...
void Audio_SetPDC(U8 PDCNum,U8 PDCMode)
{
	...
	switch(PDCMode)
	{
		case AUDIO_PDCMODESLOW:
				WriteHero_MEM_Tab(PDCSlowTab);
				break;

		case AUDIO_PDCMODEMID:   
				WriteHero_MEM_Tab(PDCMiddleTab);
				break;
		case AUDIO_PDCMODEFAST:	  
...
//const S16 ClassGeq_Table[GEQ_MAXBAND][7]=
//{
...
const S16 ClassGeq_Table[GEQ_MAXBAND][7]=
{
	{0x085,0x3E3, 0x000, 0x7C7, 0xC00, 0x028, 0x7FF, },
	{0x03E,0x513, 0x003, 0x226, 0xC07, 0x431, 0x7F8, },
	{0x000,0x1B1, 0x037, 0x362, 0xC6E, 0x009, 0x785, },
	{0x000,0x000, 0x0BE, 0x000, 0xD7C, 0x000, 0x5D9, },
	{0x12B,0x000, 0x141, 0x000, 0xE81, 0x000, 0xEA0, },
};

//const S16 PopGeq_Table[GEQ_MAXBAND][7]=
//{
...
const S16 PopGeq_Table[GEQ_MAXBAND][7]=
{
	{0x18E,0x513, 0x003, 0x226, 0xC07, 0x431, 0x7F8, },
	{0x03E,0x313, 0x00E, 0x627, 0xC1C, 0x79F, 0x7DF, },
	{0xFC8,0x653, 0x04B, 0x4A6, 0xC97, 0x1A0, 0x755, },
	{0x03E,0x000, 0x048, 0x000, 0xC91, 0x000, 0x6AD, },
	{0x03E,0x000, 0x0EF, 0x000, 0xDDD, 0x000, 0xE77, },
};

//const S16 RockGeq_Table[GEQ_MAXBAND][7]=
//{
...
const S16 RockGeq_Table[GEQ_MAXBAND][7]=
{
	{0x453,0x788, 0x001, 0x710, 0xC03, 0x7DF, 0x7FB, },
	{0x306,0x701, 0x003, 0x603, 0xC07, 0x5BF, 0x7F7, },
	{0xFC8,0x2D0, 0x027, 0x5A0, 0xC4E, 0x2BE, 0x79D, },
	{0x03E,0x000, 0x087, 0x000, 0xD0E, 0x000, 0x441, },
	{0x03E,0x000, 0x0EF, 0x000, 0xDDD, 0x000, 0xE77, },
};

const S16 News_Table[GEQ_MAXBAND][7]=
{
	{0xECC,0x1C3, 0x001, 0x385, 0xC02, 0x469, 0x7FD, },
	{0xF43,0x6B7, 0x003, 0x56E, 0xC07, 0x0EA, 0x7F8, },
	{0x27A,0x0C6, 0x044, 0x18B, 0xC88, 0x37B, 0x764, },
	{0x306,0x000, 0x087, 0x000, 0xD0E, 0x000, 0x441, },
	{0xEE5,0x000, 0x275, 0x000, 0x0EB, 0x000, 0x073, },
};

const S16 Jazz_Table[GEQ_MAXBAND][7]=
{
	{0x18E,0x513, 0x003, 0x226, 0xC07, 0x431, 0x7F8, },
	{0xF97,0x6A1, 0x01A, 0x543, 0xC35, 0x07A, 0x7C3, },
	{0x0D3,0x0C8, 0x0BE, 0x18F, 0xD7C, 0x394, 0x5D9, },
	{0xFC8,0x000, 0x0D1, 0x000, 0xDA1, 0x000, 0x3E7, },
	{0x0D3,0x000, 0x1E8, 0x000, 0xFD1, 0x000, 0xEF4, },
};

const S16 Electrical_Table[GEQ_MAXBAND][7]=
{
	{0x18E,0x1F2, 0x007, 0x3E4, 0xC0E, 0x275, 0x7F1, },
	{0x085,0x313, 0x00E, 0x627, 0xC1C, 0x79F, 0x7DF, },
	{0x085,0x0C8, 0x0BE, 0x18F, 0xD7C, 0x394, 0x5D9, },
	{0x12B,0x000, 0x0BE, 0x000, 0xD7C, 0x000, 0x3FD, },
	{0x0D3,0x000, 0x1E8, 0x000, 0xFD1, 0x000, 0xEF4, },
};

const S16 Hiphop_Table[GEQ_MAXBAND][7]=
{
	{0x27A,0x513, 0x003, 0x226, 0xC07, 0x431, 0x7F8, },
	{0x12B,0x313, 0x00E, 0x627, 0xC1C, 0x79F, 0x7DF, },
	{0xF97,0x238, 0x054, 0x470, 0xCA8, 0x541, 0x70B, },
	{0x0D3,0x000, 0x0BE, 0x000, 0xD7C, 0x000, 0x3FD, },
	{0x085,0x000, 0x141, 0x000, 0xE81, 0x000, 0xEA0, },
};

const S16 Easy_Table[GEQ_MAXBAND][7]=
{
	{0x085,0x66C, 0x005, 0x4D8, 0xC0B, 0x219, 0x7F4, },
	{0xF97,0x059, 0x012, 0x0B2, 0xC24, 0x52C, 0x7D8, },
	{0x03E,0x286, 0x048, 0x50D, 0xC90, 0x798, 0x6AC, },
	{0x000,0x000, 0x141, 0x000, 0xE81, 0x000, 0x35D, },
	{0x0D3,0x000, 0x141, 0x000, 0xE81, 0x000, 0xEA0, },
};

const S16 Country_Table[GEQ_MAXBAND][7]=
{
	{0x085,0x690, 0x001, 0x520, 0xC03, 0x275, 0x7FC, },
	{0x03E,0x313, 0x00E, 0x627, 0xC1C, 0x79F, 0x7DF, },
	{0x03E,0x1C1, 0x023, 0x382, 0xC46, 0x430, 0x7A5, },
	{0x0D3,0x000, 0x087, 0x000, 0xD0E, 0x000, 0x441, },
	{0x0D3,0x000, 0x141, 0x000, 0xE81, 0x000, 0xEA0, },
};
...
S8 Audio_SetEQMode(U8 GeqMode)
{
	...
	else if(GeqMode <= HERO_GEQMODE_CLASS)//preset mode
	{
		const S16 (*ptab)[7];
		...
		switch(GeqMode){
			...
			case HERO_GEQMODE_ROCK:
				...
				break;
			case HERO_GEQMODE_NEWS:
				{
					Data = /*0x0453*/table_dB2Lin[10];//the descale data
					ptab = News_Table;//get the para table pointer
				}
				break;
			case HERO_GEQMODE_JAZZ:
				{
					Data = /*0x0453*/table_dB2Lin[10];//the descale data
					ptab = Jazz_Table;//get the para table pointer
				}
				break;
			case HERO_GEQMODE_ELECDANCE:
				{
					Data = /*0x0453*/table_dB2Lin[10];//the descale data
					ptab = Electrical_Table;//get the para table pointer
				}
				break;
			case HERO_GEQMODE_HIPHOP:
				{
					Data = /*0x0453*/table_dB2Lin[10];//the descale data
					ptab = Hiphop_Table;//get the para table pointer
				}
				break;
			case HERO_GEQMODE_EASY:
				{
					Data = /*0x0453*/table_dB2Lin[10];//the descale data
					ptab = Easy_Table;//get the para table pointer
				}
				break;
			case HERO_GEQMODE_COUNTRY:
				{
					Data = /*0x0453*/table_dB2Lin[10];//the descale data
					ptab = Country_Table;//get the para table pointer
				}
				break;
		}
...
void Audio_GEQUserConfigInit(void)
{
	...
	Geq_ActiveBand=0;
#if (GEQ_9BAND_ENABLE)
     Geq_Para[GEQ_BAND1].Fc=50;//set the default band freq
	Geq_Para[GEQ_BAND2].Fc=100;
	Geq_Para[GEQ_BAND3].Fc=200;
	Geq_Para[GEQ_BAND4].Fc=400;
	Geq_Para[GEQ_BAND5].Fc=800;
	Geq_Para[GEQ_BAND6].Fc=1600;
	Geq_Para[GEQ_BAND7].Fc=3200;
	Geq_Para[GEQ_BAND8].Fc=6400;
	Geq_Para[GEQ_BAND9].Fc=12800;	
#else
     Geq_Para[GEQ_BAND1].Fc=50;//set the default band freq
	Geq_Para[GEQ_BAND2].Fc=200;
	Geq_Para[GEQ_BAND3].Fc=800;
	Geq_Para[GEQ_BAND4].Fc=3200;
	Geq_Para[GEQ_BAND5].Fc=12800;
#endif
	Geq_AcualMaxGain=0;
	Geq_DescaleData=0x07FF;
	for(band=GEQ_BAND1;band<GEQ_MAXBAND;band++)
	{
		Geq_Para[band].Fs=TEF663x_ADSP_SAMPLEFREQ;
		Geq_Para[band].Q=1.0;
		Geq_Para[band].Gain=0.0;
...
void Audio_SetPrimaryInputSource(eAudio_InputSource InputSource)
{
	...
	Audio_SourceInputSel(PrimaryStr,InputSource,0);
}
