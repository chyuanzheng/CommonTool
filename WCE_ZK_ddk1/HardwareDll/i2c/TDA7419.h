...
public:
	...
	enum
	{
		...
		SUB_SpeakerLR,
		SUB_SpeakerRR,
		SUB_MixingLevelControl,
		SUB_SubwooferAtten,
		SUB_SpectrumAnalyzer,
		SUB_Testing
	};
	...
	BOOL Volume(BYTE vol, BYTE softstep = 0)
	{
		...
		// 混音音量比主音量略高
		WriteIIC(SUB_MixingLevelControl, dataMix);
	...
	void SecondSelector(BYTE source, BYTE inputgain = 0, BYTE rearsource = 0)
	{
		BYTE data = (source & 0x7) | (inputgain<<3 & 0xF<<3) | (rearsource<<7);
		WriteIIC(SUB_SecondSourceSelector, data);
	}
	void Subwoofer(BYTE subwoofer=0, BYTE middlefreq=0, BYTE bassfreq=0, BYTE dcmode=0, BYTE smoothfiler=0)
	{
		BYTE data = subwoofer | middlefreq<<2 | bassfreq<<4 | dcmode<<6 | smoothfiler<<7;
		WriteIIC(SUB_Subwoofer, data);
	}
	void Mixer(BYTE mixon, BYTE subwooferoff = 1, BYTE gain = 0)
	{
		BYTE data;
		if (mixon)
		{
			data = 0x8 | subwooferoff<<3 | gain<<4;
			WriteIIC(SUB_Mixing, data);
		}
		else
		{
			data = 0xF | subwooferoff<<3 | gain<<4;
			WriteIIC(SUB_Mixing, data);
		}
	}
	...
	void MixerVol(BYTE atten, BYTE softstep=0)
	{
		atten = MixVolTable[atten];
		BYTE data = atten | (softstep<<7);
		WriteIIC(SUB_MixingLevelControl, data);
	}
	void Init()
	{
		BYTE data[18] = {
