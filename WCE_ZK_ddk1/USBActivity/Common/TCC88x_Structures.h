...
************************************************************************/
#ifndef __TCC88xSTRUCTURES_H__
#define __TCC88xSTRUCTURES_H__
...
typedef struct _PIC{
    ...
    volatile unsigned int   EI37SEL;            // 0x080  R/W  0x00000000  External Interrupt 3~7 Selection Register
...
typedef struct _GPIO{
	...
	volatile unsigned int	ECID0;					//	 0x190	R/W	0x00000000	CID Output Register
	volatile unsigned int	ECID1;					//	 0x194	R/W		-		CID Serial Input Register
	volatile unsigned int	ECID2;					//	 0x198	R/W		-		CID Parallel Input 0 Register
	volatile unsigned int	ECID3;					//	 0x19C	R/W		-		CID Parallel Input 1 Register
	volatile unsigned int	Reserved1[8];			//	 0x1A0 ~ 0x1BF
...
typedef struct _SMUI2C{
	...
	volatile unsigned int  SR1;                  // 0x54 R 0x0000 Status register
	volatile unsigned int  TIME1;                // 0x58 R/W 0x0000 Timing Control Register    
	volatile unsigned int  NOTUSING1[9];	
...
typedef struct _LCDC{
    ...
    volatile unsigned int   LI3C;               //  0x0F0   R/W     0x00000000  LCD Image 3 Control Register
    volatile unsigned int   LI3P;               //  0x0F4   R/W     0x00000000  LCD Image 3 Position Register
    volatile unsigned int   LI3S;               //  0x0F8   R/W     0x00000000  LCD Image 3 Size Register
    volatile unsigned int   LI3BA0;             //  0x0FC   R/W     0x00000000  LCD Image 3 Base Address 0 Register.
    volatile unsigned int   LI3CA;              //  0x100   R/W     0x00000000  LCD Image 3 Current Address Register.
    volatile unsigned int   LI3BA1;             //  0x104   R/W     0x00000000  Not Used
    volatile unsigned int   LI3BA2;             //  0x108   R/W     0x00000000  Not Used
    volatile unsigned int   LI3O;               //  0x10C   R/W     0x00000000  LCD Image 3 Offset Register
    volatile unsigned int   LI3SR;              //  0x110   R/W     0x00000000  LCD Image 3 Scale ratio
    volatile unsigned int   LI3A;               //  0x114   R/W     0x00000000  LCD Image 3 Alpha Register
    volatile unsigned int   LI3KR;              //  0x118   R/W     0x00000000  LCD Image 3 Keying Register for RED or LUMA(Y)
    volatile unsigned int   LI3KG;              //  0x11C   R/W     0x00000000  LCD Image 3 Keying Register for BLUE or CHROMA(Cb)
    volatile unsigned int   LI3KB;              //  0x120   R/W     0x00000000  LCD Image 3 Keying Register for GREEN or CHROMA(Cr)
    volatile unsigned int   LI3EN;              //  0x124   R/W     0x00000020  LCD Image 3 Enhancement Register
    volatile unsigned int   LUTIDX;             //  0x128   R/W     0x00000070  Lookup Table index Register
} LCDC, *PLCDC;
...
typedef struct _VIQE{
	...
	volatile unsigned int _NOTDEFINE15[36];
    volatile unsigned int HI_LUTS[64];          // 0x700 ~ 0x7FC  R/W  0x00000000 Histogram LUT Table Register
	volatile unsigned int _NOTDEFINE16[512];
	volatile unsigned int _NOTDEFINE17[1024];
}VIQE, *PVIQE;
...
typedef struct _DDICONFIG{
    ...
    volatile unsigned int   NOTDEFINE0[10]  ;   //  0x004~0x028 R/W -       -
...
typedef struct _CAMBUSCFG {
	...
	volatile unsigned int	SWRESET;			//0x04			RW	
	volatile unsigned int	CLKO_SEL;			//0x08			RW	
	volatile unsigned int	MEM_EMA;			//0x0C			RW	
	volatile unsigned int	Reserved0[4];		//0x10 ~ 0x1F	-	
	volatile unsigned int	ISP_RC_CTRL;		//0x20			RW	
	volatile unsigned int	ISP_RC_A0_MIN;		//0x24			RW	
	volatile unsigned int	ISP_RC_A0_MAX;		//0x28			RW	
	volatile unsigned int	CIF_RC_CTRL;		//0x2C			RW	
	volatile unsigned int	CIF_RC_A0_MIN;		//0x30			RW	
	volatile unsigned int	CIF_RC_A0_MAX;		//0x34			RW	
} CAMBUSCFG, *PCAMBUSCFG;

#endif /*__TCC88xSTRUCTURES_H__*/
