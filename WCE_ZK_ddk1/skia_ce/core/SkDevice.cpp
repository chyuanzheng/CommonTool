...
bool SkDevice::getPath(const SkDraw& draw, const void* text, size_t len, SkScalar x, SkScalar y, const SkPaint& paint, std::vector<SkPath> &path)
{
	return draw.getPath((const char*)text, len, x, y, paint,path);
}
