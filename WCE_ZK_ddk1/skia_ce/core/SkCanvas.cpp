...
bool  SkCanvas::getPaths(const void* text, size_t byteLength,
			   SkScalar x, SkScalar y, const SkPaint& paint, std::vector<std::vector<SkPath>>& ps)
{
	std::vector<SkPath> path;
	LOOPER_BEGIN(paint, SkDrawFilter::kText_Type)

		while (iter.next()) {
			SkDeviceFilteredPaint dfp(iter.fDevice, looper.paint());
			//iter.fDevice->drawText(iter, text, byteLength, x, y, dfp.paint());
			bool b = iter.fDevice->getPath(iter, text, byteLength, x, y, dfp.paint(),path);
			if (b)
			{
				ps.push_back(path);
			}
