...
void SkDraw::drawPathToPath(const SkPath& origSrcPath, const SkPaint& paint,
                      const SkMatrix* prePathMatrix, bool pathIsMutable, SkPath& resultPath) const {
    SkDEBUGCODE(this->validate();)
...
void SkDraw::drawText_toPaths(const char text[], size_t byteLength,
                              SkScalar x, SkScalar y,
                              const SkPaint& paint, std::vector<SkPath> &path) const {
    SkDEBUGCODE(this->validate();)
    ...
    while ((iterPath = iter.next(&xpos)) != NULL) {
		...
		// this->drawPath(*iterPath, pnt, &matrix, false);
		 //this->drawPath(*iterPath, pnt, &matrix, false);
		this->drawPathToPath(*iterPath, pnt, &matrix, false,dest);
		path.push_back(dest);
    }
}
...
///////////////////////////////////////////////////////////////////////////////
bool    SkDraw::getPath(const char text[], size_t byteLength, SkScalar x,
						SkScalar y, const SkPaint& paint,std::vector<SkPath> &path) const {
    ...
    if (/*paint.isLinearText() ||*/
        /*(fMatrix->hasPerspective())*/true) {
        this->drawText_toPaths(text, byteLength, x, y, paint,path);
